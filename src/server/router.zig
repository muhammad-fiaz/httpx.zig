...
pub const Router = struct {
    ...
    /// Finds a matching route for the given method and path.
    ///
    /// Note: The returned `params` slice points to the router's internal `params_buf`
    /// to avoid per-match heap allocations. Concurrent calls to `find()` on the same
    /// `Router` instance are therefore not thread-safe without external synchronization;
    /// callers in concurrent environments should copy extracted parameters if retained.
    pub fn find(self: *Self, method: types.Method, path: []const u8) ?struct { handler: Handler, params: []const RouteParam } {
        for (self.routes.items) |route| {
            if (route.method != method) continue;
