...
//! attributes (quoted/unquoted/boolean), source range tracking, and graceful error recovery.
            ...
            if (src[i] != '<') {
                ...
                if (raw.len > 0) {
                    ...
                    const idx = try self.tree.append(self.arena, .{
                        .kind = .text,
                        .data = raw,
                        .range = makeRange(src, start, i),
                    });
                    self.tree.appendChild(self.currentParent(), idx);
                }
                continue;
            }
            const tag_open_idx = i;
            i += 1;
            if (i >= src.len) break;
