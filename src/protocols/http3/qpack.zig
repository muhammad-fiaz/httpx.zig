...
pub const Encoder = struct {
    ...
    pub fn encodeField(
        ...
        try out.append(self.allocator, 0x00); // S=0 R=0 N=0 -> literal never-indexed
...
pub const Decoder = struct {
    ...
    pub fn decodeSection(self: *Decoder, data: []const u8) Error![]FieldLine {
        ...
        // Note: this simplified codec omits the Encoded Field Section Prefix
        // (Required Insert Count + Base). Full dynamic-table support layers on top.
        ...
        while (offset < data.len) {
            ...
            if (first & 0x80 != 0) {
                ...
                // Dynamic-table name references require connection state that
                // this decoder does not expose; never reinterpret them as
                // static references.
                return Error.InvalidInstruction;
            } else {
                // Fallback: treat as literal like HPACK style
                // Consume the field-line instruction byte before reading the
                // name length that follows it.
                offset += 1;
    ...
    ///
    /// Dynamic-table state is not yet exposed by this decoder. Consequently,
    /// a non-zero Required Insert Count or Delta Base is rejected rather than
    /// treating a dynamic reference as a static one. This keeps the boundary
    /// safe while allowing static-only HTTP/3 HEADERS blocks to use the real
    /// wire format.
    pub fn decodeSectionWithPrefix(self: *Decoder, data: []const u8) Error![]FieldLine {
        ...
        if (required_insert_count != 0) return Error.InvalidInstruction;

        if (offset >= data.len) return Error.InvalidInstruction;
        const sign_and_delta = data[offset];
        const delta_base = try decodeInt(data, &offset, 7);
        if (sign_and_delta & 0x80 != 0 or delta_base != 0) return Error.InvalidInstruction;
